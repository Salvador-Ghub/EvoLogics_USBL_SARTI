...
for i=1:1:length(PuntosAng)
    switch i
        ...
        case 2
            form='*';
        case 3
            form='o';clear all;%close all;
format long
tic
cs = 1500;%velocidad del sonido
double CTang
double CTdelayang
%iniciamos las posiciones de las antenas iz, dr, ar, ab.
dreal=47.5*10^-3;
...
PuntosAng=[C1;C2;C3;C4;C5];%juntamos posiciones en un array
errora=0;
error_ms=0.000000%5;%error en milisegundos 
PosTag=[2000 1900 2000];%%%Posición del emisor
PosTagReal=PosTag;
RanguloXZ=rad2deg(atan(PosTag(3)/(sqrt(PosTag(1)^2+PosTag(2)^2))))
...
scatter3(PosTag(1),PosTag(2),PosTag(3));hold on;
Aang=PosTag-PuntosAng;%resta de las posiciones por coordenadas respecto al emisor
CTang=(vecnorm(Aang')/cs);%calcula la distancia de cada punto al emisor dividido entre la velocidad del sonido

CTang=[CTang(1)+rand*error_ms/1000 CTang(2)+rand*error_ms/1000 CTang(3)+rand*error_ms/1000 CTang(4)+rand*error_ms/1000 CTang(5)+rand*error_ms/1000];%valores de error aplicados

CTdelayang=((CTang)-max(CTang))*(cs*1);%interpretamos esta distancia como los retardos de señal
...
errorTnA=10;%valor de error imaginario grande
        ...
        for mi=1:7
            ...
            CTangA=vecnorm(AangA')/cs;
            CTdelayangS=(CTangA-max(CTangA))*(cs*1);%delay simulado
...
%%funcion punto anterior
     ...
     CTangB=vecnorm(AangB')/cs;
     CTdelayangS=(CTangB-max(CTangB))*(cs*1);%delay simulado
     error=CTdelayangS-CTdelayang;
